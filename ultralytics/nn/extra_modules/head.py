...
class Detect_DyHead_Prune(nn.Module):
    ...
    def __init__(self, nc=80, hidc=256, block_num=2, ch=()):  # detection layer
        ...
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], self.nc)  # channels
        self.conv = nn.ModuleList(nn.Sequential(Conv(x, hidc, 1)) for x in ch)
        self.dyhead = nn.Sequential(*[DyHeadBlock_Prune(hidc) for i in range(block_num)])
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(hidc, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for _ in ch)
        self.cv3 = nn.ModuleList(nn.Sequential(Conv(hidc, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for _ in ch)
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            x[i] = self.conv[i](x[i])
        for dyhead_layer_num in range(len(self.dyhead)):
            new_x = []
            for i in range(self.nl):
                new_x.append(self.dyhead[dyhead_layer_num](x, i))
            x = new_x
        shape = x[0].shape  # BCHW
        for i in range(self.nl):
            x[i] = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1)
